...
def create_extraction_prompt(markdown_content, json_template):
    ...
    prompt = f"""Ты помощник для извлечения данных из резюме. Твоя задача - заполнить JSON структуру данными из предоставленного текста резюме.

КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
1. НЕ добавляй никакой новой информации, которой нет в исходном тексте
2. НЕ придумывай детали, даты, названия компаний, проекты или другие данные
3. НЕ используй внешние знания или данные из интернета
4. Извлекай ТОЛЬКО данные, которые явно указаны в тексте
5. Если информация отсутствует в тексте, оставляй поле пустым (пустая строка "" или пустой массив [])
6. Сохраняй все даты, названия, контакты, навыки точно как в оригинале
7. Для массивов используй только данные из текста, не добавляй свои предположения

СПЕЦИАЛЬНЫЕ ИНСТРУКЦИИ ДЛЯ ОТДЕЛЬНЫХ ПОЛЕЙ:

ПОЛЕ "full_name" (ФИО):
- Извлеки полное имя (ФИО) из резюме
- Ищи имя в начале резюме, в заголовке или в контактной информации
- Если ФИО не указано явно, оставь поле пустым

ПОЛЕ "project_background" (проектный бекграунд):
- Укажи 1-3 отрасли/домена, в которых работал кандидат (финтех, банки, ритейл и т.п.)
- Формат: единая строка в полужирном формате (**текст**) через запятую, без предложений
- Не указывай тип разработки (backend, full-stack и т.п.), только сферы/домены
- Используй информацию из резюме о проектах и клиентах, не придумывай ничего нового

ПОЛЕ "pitch" (рассказ о себе):
- В приоритете используй уже имеющийся текст "о себе"/резюме, без выдуманных деталей; если его нет, сделай минимальный сжатый текст
- Пиши от первого лица, но без явного местоимения "я" в начале фраз (начинай с действий или кратких описаний)
- Размер: 3-5 предложений
- Используй только информацию о реальном опыте, навыках и достижениях из резюме
- Формулируй естественно и профессионально

ПОЛЕ "soft_skills" (в разделе screening):
- Извлеки soft skills (мягкие навыки) из резюме
- Ищи упоминания о личностных качествах, коммуникативных навыках, лидерстве, работе в команде и т.д.
- Если в резюме явно указаны soft skills - используй их
- Если soft skills не указаны явно, но можно вывести из описания опыта работы или проектов - извлеки их
- Примеры: "командная работа", "лидерство", "коммуникабельность", "стрессоустойчивость", "адаптивность"
- НЕ придумывай soft skills, которых нет в тексте или которые нельзя логически вывести из опыта

ПОЛЕ "skills_and_tools" (навыки и инструменты в general_info):
- Это поле ТОЛЬКО про технологии/языки/инструменты, которые кандидат действительно использовал
- Не добавляй сюда учебные курсы, вузы, платформы или названия программ
- Формат каждого элемента: "Категория: значение1, значение2" (как в примере "Языки программирования и скриптовые языки: Python, Bash, SQL")
- Категории подбирай по содержимому резюме (не выдумывай технологии). Возможные варианты:
  ...
  * "Асинхронные очереди и шины сообщений"
  * "Другое"
- Если технологий мало, используй только подходящие категории, без лишних
- Источник только резюме, никаких придуманных технологий

ПОЛЕ "education" (образование в general_info):
- Только формальное образование (без курсов): университет, специальность, город/страна, год
- Формат каждого элемента: "ГОД, Университет, город\nФакультет/направление, Специальность (степень)"
- Пример: "2015, Московский финансово-юридический университет (Московская финансово-юридическая академия), Москва\nФакультет бизнеса и информационных технологий, Программное обеспечение вычислительной техники и автоматизированных систем (инженер)"
- Используй только факты из резюме, ничего не выдумывай

НОВОЕ ПОЛЕ "advanced_training" (Курсы повышения квалификации в general_info):
- Список курсов повышения квалификации/доп.образования
- Формат каждого элемента: "ГОД г., Название курса — Организация" (если организации нет, оставь только год и название)
- Примеры:
  * "2020 г., Моделирование в ArchiMate — Управление по Agile: Scrum, Kanban, Lean"
  * "2020 г., Управление по Agile: Scrum, Kanban, Lean — Нетология"
  * "2025 г., Архитектура программного обеспечения — Яндекс Практикум"
- Не дублируй здесь университеты/основное образование, не придумывай новые курсы

ПОЛЕ "technologies" (в work_experience) и "technologies_and_tools" (в project_experience):
- Извлеки технологии, используемые в конкретном проекте или на конкретном месте работы
- Форматируй КАЖДОЕ значение как простую строку без категорий и пояснений
- Если в тексте указано "Фреймворки: Flask, Django", записывай только "Flask" и "Django" как отдельные элементы массива
- Разделяй технологии на отдельные элементы массива (не объединяй в одну строку через точку); если внутри одного элемента нужно перечисление, используй запятую, не точку с запятой
- НЕ придумывай технологии, которых нет в описании проекта или работы; если технология не указана явно, можно аккуратно восстановить её из других частей резюме, но без добавления новых

ПОЛЕ "achievements" (в каждом объекте work_experience, work_experience[].projects и project_experience):
- Заполняй списком конкретных результатов и достижений (улучшения метрик, рост показателей, внедрение решений)
- Используй только факты из текста резюме, ничего не придумывай
- Если достижений нет, оставляй пустой массив []

ДОПОЛНИТЕЛЬНЫЕ ТРЕБОВАНИЯ ДЛЯ "project_experience":
- company: только название места работы/проекта
- period: промежуток времени в формате "ГОД МЕСЯЦ - ГОД МЕСЯЦ" (или "с ГОД МЕСЯЦ"), всегда указывай
- role: всегда заполняй, даже если нужно взять роль из описания задач/должности
- achievements: указывай, если есть в тексте; не придумывай (если нет, оставь пустой массив, но поле сохраняй)
- technologies_and_tools: перечисление технологий обязательно, используй запятые, а не точку с запятой; можно извлечь из других частей резюме, но не добавляй новые технологии

Структура JSON, которую нужно заполнить:
{template_str}

Исходный текст резюме:
{markdown_content}

Заполни JSON структуру данными из резюме. Верни ТОЛЬКО валидный JSON без дополнительных комментариев или объяснений. Если какая-то информация отсутствует в тексте, оставь поле пустым."""
            ...
            for key, value in source.items():
                if key in result:
                    if isinstance(value, dict) and isinstance(result[key], dict):
                        ...
                        # Для списков берем данные из source, если они не пустые
                        result[key] = value if value else result[key]
                    else:
                        # Для примитивных значений берем из source, если не пустое
                        result[key] = value if value else result[key]
                else:
                    result[key] = value
            return result
